...
class DQN_Agent():
    ...
    def __init__(self, environment_name, sess, gridgraph, render=False):
        ...
        if environment_name == 'grid':
            self.gamma = 0.95
        self.max_episodes = 500  # 20000 #200
        self.batch_size = 32
        self.render = render
    ...
    def train(self, savepath, model_file=None):
        ...
        for episode in np.arange(self.max_episodes * len(self.gridgraph.twopin_combo)):
            ...
            state, reward_plot,isBest = self.gridgraph.reset()
            ...
            while not is_terminal:
                ...
                q_=np.zeros(self.batch_size)
                for i in range(self.batch_size):
                    q_[i]=q_batch[i][batch_action[i]]
                TDerror=y_batch-q_
                TDerror=np.abs(TDerror)
                ...
                self.replay.batch_update(batch_idx,TDerror)
...
if __name__ == '__main__':
    ...
    model_path = './prioritizedReplayModel/'
    data_path = './prioritizedReplayData/'
    solutionPicture_path='./prioritizedReplaySolutionPicture/'
    solutionResult_path='./prioritizedReplaySolutionResult/'
