...
import math
import os
import GridGraph_Reroute as env
    ...
    def train(self, savepath, model_file=None):
        ...
        for episode in np.arange(self.max_episodes * len(self.gridgraph.twopin_combo)):
            ...
            if self.gridgraph.clearCapacityFlag==1:
                ...
                ## 存布线结果
                if(len(self.gridgraph.recentPassby)<len(self.gridgraph.netOrder)):
                    self.gridgraph.recentPassby.append(self.gridgraph.passby)
                else:
                    self.gridgraph.recentPassby[self.gridgraph.net_ind]=self.gridgraph.passby
                self.gridgraph.passby=np.zeros_like(self.gridgraph.capacity)
                self.gridgraph.clearCapacityFlag=0
                self.gridgraph.net_ind += 1
                ## 拆下一线网
                # print(self.gridgraph.capacity)
                if(len(self.gridgraph.recentPassby)==len(self.gridgraph.netOrder)):
                    # print(1)
                    self.gridgraph.capacity += self.gridgraph.recentPassby[self.gridgraph.net_ind % len(self.gridgraph.netOrder)]
                # print(gridgraph.capacity)
