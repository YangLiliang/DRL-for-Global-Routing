...
class GridGraph(object):
    def __init__(self, gridParameters, agentname = None):
        ...
        self.twoPinPointer=0
        self.passbyBuffer=[]
        self.currentStateBuffer=[]
        self.currentStepBuffer=[]
        self.routeBuffer=[]
        self.switchPassbyFlag=0
        return
    ...
    def step(self, action):  # used for DRL
        ...
        if action == 0 and (self.capacity[state[0], state[1], state[2] - 1, 0] > 0 or self.passby[state[0], state[1], state[2] - 1, 0] == 1):
            ...
            #     reward = 0.0
            # else:
            #     reward += (self.capacity[state[0], state[1], state[2] - 1, 0] * 1.0 - 2) / 4
            ...
            #     reward = 0.0
            # else:
            #     reward += (self.capacity[state[0], state[1], state[2] - 1, 1] * 1.0 - 2) / 4
            nextState = (state[0] - 1, state[1], state[2], state[3] - self.gridParameters['tileWidth'], state[4])
            ...
            #     reward = 0.0
            # else:
            #     reward += (self.capacity[state[0], state[1], state[2] - 1, 2] * 1.0 - 2) / 4
            ...
            #     reward = 0.0
            # else:
            #     reward += (self.capacity[state[0], state[1], state[2] - 1, 3] * 1.0 - 2) / 4
            nextState = (state[0], state[1] - 1, state[2], state[3], state[4] - self.gridParameters['tileHeight'])
    ...
    def reset(self):
        ...
        if self.positivePin > self.posTwoPinNum:
            ...
            self.best_route = self.routeBuffer
    ...
    def initBuffers(self):
        self.passbyBuffer=[]
        for i in range(len(self.twopin_combo)):
            self.passby=np.zeros_like(self.capacity)
            self.passbyBuffer.append(self.passby)
        self.currentStateBuffer=[self.twopin_combo[i][0] for i in range(len(self.twopin_combo))]
        self.currentStepBuffer=[0]*len(self.twopin_combo)
        self.routeBuffer = [0]*len(self.twopin_combo)


    def switch(self,twoPinPointer):
        self.twoPinPointer=twoPinPointer
        self.init_state=self.twopin_combo[twoPinPointer][0]
        self.goal_state=self.twopin_combo[twoPinPointer][1]

        self.current_state=self.currentStateBuffer[twoPinPointer]
        self.current_step=self.currentStepBuffer[twoPinPointer]
        if self.routeBuffer[twoPinPointer] == 0:
            self.route=[]
        else:
            self.route=self.routeBuffer[twoPinPointer]

        pt = twoPinPointer + 1
        for netPointer in range(len(self.twopin_combo)):
            pt = pt - self.net_pair[netPointer]
            if pt <= 0:
                self.passby = self.passbyBuffer[netPointer]
                break

        return self.state2obsv()

    def saveBuffer(self,twoPinPointer):

        self.currentStateBuffer[twoPinPointer]=self.current_state
        self.currentStepBuffer[twoPinPointer]=self.current_step
        self.routeBuffer[twoPinPointer]=self.route

        pt = twoPinPointer + 1
        for netPointer in range(len(self.twopin_combo)):
            pt = pt - self.net_pair[netPointer]
            if pt <= 0:
                self.passbyBuffer[netPointer]=self.passby
                break
    ...
    def showCapacity(self, filename=None):
        ...
        layer1_y = self.capacity[:, 0:self.gridParameters['gridSize'][1] - 1, 0, 2]
        layer2_x = self.capacity[0:self.gridParameters['gridSize'][0] - 1, :, 1, 0]
        layer2_y = self.capacity[:, 0:self.gridParameters['gridSize'][1] - 1, 1, 2]
        # print(layer1_x)
        # print(layer1_y)
        # print(layer2_x)
        # print(layer2_y)
        sns.heatmap(layer1_x, cmap='Reds')
        if filename != None:
        ...
        # plt.show()
        sns.heatmap(layer2_y, cmap='Reds')
        if filename != None:
