...
N_S=env.observation_space  #观测值个数
N_A=env.action_space  #行为值个数
...
class ACnet(object):     #这个class即可用于生产global net，也可生成 worker net，因为结构相同
    ...
    def build_net(self,scope): #建立神经网络过程
        ...
        with tf.variable_scope('actor'):            #actor神经网络结构
            l_a=tf.layers.dense(inputs=self.s,units=32,activation=tf.nn.relu6,
                                kernel_initializer=w_init,bias_initializer=tf.constant_initializer(0.1),name='la')  #建立第一层神经网络
            l_b=tf.layers.dense(inputs=l_a,units=64,activation=tf.nn.relu6,
                                kernel_initializer=w_init,bias_initializer=tf.constant_initializer(0.1),name='lb')  #建立第二层神经网络
            l_c=tf.layers.dense(inputs=l_b,units=32,activation=tf.nn.relu6,
                                kernel_initializer=w_init,bias_initializer=tf.constant_initializer(0.1),name='lc')  #建立第三层神经网络
            acts_prob=tf.layers.dense(inputs=l_c,units=N_A,activation=tf.nn.softmax,
                               kernel_initializer=w_init,bias_initializer=tf.constant_initializer(0.1),name='act_prob')  #第四层神经网络其中之一输出为动作的均值

        with tf.variable_scope('critic'):     #critic神经网络结构,输入为位置的观测值，输出为评价值v
            l_a=tf.layers.dense(self.s,4,tf.nn.relu6,kernel_initializer=w_init,bias_initializer=tf.constant_initializer(0.1),name='la')  #建立第一层神经网络
            l_b=tf.layers.dense(l_a, 8, tf.nn.relu6, kernel_initializer=w_init,bias_initializer=tf.constant_initializer(0.1), name='lb')  # 建立第二层神经网络
            l_c = tf.layers.dense(l_b, 4, tf.nn.relu6, kernel_initializer=w_init,bias_initializer=tf.constant_initializer(0.1), name='lc')  # 建立第三层神经网络
            v=tf.layers.dense(l_c,1,kernel_initializer=w_init,bias_initializer=tf.constant_initializer(0.1),name='v')   #第四层神经网络
...
class Worker(object):
    def __init__(self,name,globalAC, gridparameters):    #传入的name是worker的名字，globalAC是已经建立好的中央大脑GLOBALE—AC
        self.env=env.GridGraph(gridparameters)
        self.burnInEnv=env.GridGraph(gridparameters)
    ...
    def work(self):   #定义worker运行的的具体过程
        ...
        while not COORD.should_stop() and GLOBALE_EP<MAX_GLOBAL_EP:   #停止本worker运行的条件
            ...
            ep_r = 0  # 本回合总的reward
            for loop in range(self.env.twopinNum):  #本循环一次是一个回合
            ...
            print(self.name,' EP:',GLOBALE_EP,' reword:',ep_r)
            GLOBALE_EP+=1       #加一回合
...
if __name__=='__main__':
    ...
    with tf.device('/cpu:0'):
        ...
        for i in range(N_workers):    #N—workers等于cpu数量
            ...
            workers.append(Worker(name=i_name,globalAC=GLOBALE_AC, gridparameters=gridParameters))   #创建独立的worker
