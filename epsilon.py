...
class DQN_Agent():
    ...
    def epsilon_greedy_policy(self, q_values):
        ...
        rnd = np.random.rand()
        if(self.iterator<=self.max_episodes/10):
            self.epsilon=0.20
        elif (self.iterator > self.max_episodes / 10 and self.iterator <= self.max_episodes * 3 / 10):
            self.epsilon = 0.10
        elif(self.iterator>self.max_episodes*3/10 and self.iterator<=self.max_episodes*8/10):
            self.epsilon=0.06
        elif (self.iterator > self.max_episodes*8 / 10):
            self.epsilon = 0.04
    ...
    def train(self, savepath, model_file=None):
        ...
        for episode in np.arange(self.max_episodes * len(self.gridgraph.twopin_combo)):
            ...
            if (episode % len(self.gridgraph.twopin_combo) == 0):
                self.iterator += 1
            # print(self.iterator)
        ...
        saver.save(self.sess, model_path + 'my_net.ckpt')
